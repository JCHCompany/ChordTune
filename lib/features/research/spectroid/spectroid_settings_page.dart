...
class SpectroidSettingsPage extends StatefulWidget {
  ...
  const SpectroidSettingsPage({super.key, required this.initialConfig, required this.onApply});
      ...
      body: ListView(
        ...
        children: [
          ...
          SwitchListTile(
            ...
            Row(children:[const SizedBox(width:160, child: Text('YIN seuil')), Expanded(child: Slider(min:0.05,max:0.3,divisions:25,value:_cfg.yinThreshold.clamp(0.05,0.3), label:_cfg.yinThreshold.toStringAsFixed(2), onChanged:(v)=> setState(()=> _cfg=_cfg.copyWith(yinThreshold:v))))]),
            Row(children:[const SizedBox(width:160, child: Text('Harmoniques (H)')), Expanded(child: Slider(min:3,max:12,divisions:9,value:_cfg.harmH.toDouble().clamp(3,12), label:_cfg.harmH.toString(), onChanged:(v)=> setState(()=> _cfg=_cfg.copyWith(harmH:v.round()))))]),
            Row(children:[const SizedBox(width:160, child: Text('Tolérance (cents)')), Expanded(child: Slider(min:5,max:50,divisions:45,value:_cfg.harmTolCents.clamp(5,50), label:_cfg.harmTolCents.toStringAsFixed(0), onChanged:(v)=> setState(()=> _cfg=_cfg.copyWith(harmTolCents:v))))]),
            Row(children:[const SizedBox(width:160, child: Text('Décroissance harm.')), Expanded(child: Slider(min:0.5,max:0.95,divisions:45,value:_cfg.harmWeightDecay.clamp(0.5,0.95), label:_cfg.harmWeightDecay.toStringAsFixed(2), onChanged:(v)=> setState(()=> _cfg=_cfg.copyWith(harmWeightDecay:v))))]),
            Row(children:[const SizedBox(width:160, child: Text('Poids YIN')), Expanded(child: Slider(min:0,max:1,divisions:20,value:_cfg.fusionYinWeight.clamp(0,1), label:_cfg.fusionYinWeight.toStringAsFixed(2), onChanged:(v)=> setState(()=> _cfg=_cfg.copyWith(fusionYinWeight:v))))]),
            Row(children:[const SizedBox(width:160, child: Text('Poids Harm')), Expanded(child: Slider(min:0,max:1,divisions:20,value:_cfg.fusionHarmWeight.clamp(0,1), label:_cfg.fusionHarmWeight.toStringAsFixed(2), onChanged:(v)=> setState(()=> _cfg=_cfg.copyWith(fusionHarmWeight:v))))]),
          ...
          Row(
            children: [
              const SizedBox(width: 160, child: Text('Décimations (échelle 2^k)')),
              Expanded(
                child: DropdownButton<int>(
                  isExpanded: true,
                  value: _cfg.decimLevels,
                  items: List.generate(10, (i) => i)
                      .map((d) => DropdownMenuItem<int>(value: d, child: Text('$d (×${1 << d})')))
                      .toList(),
                  onChanged: (v) => setState(() => _cfg = _cfg.copyWith(decimLevels: v ?? _cfg.decimLevels)),
                ),
              ),
            ],
          ),
            ...
            children: [
              ...
              Expanded(
                child: DropdownButton<LowFreqHighPass>(
                  ...
                  items: const [
                    DropdownMenuItem(value: LowFreqHighPass.off, child: Text('OFF')),
                    DropdownMenuItem(value: LowFreqHighPass.hz0p5, child: Text('0.5 Hz')),
                    DropdownMenuItem(value: LowFreqHighPass.hz1, child: Text('1 Hz (défaut)')),
                    DropdownMenuItem(value: LowFreqHighPass.hz5, child: Text('5 Hz')),
                    DropdownMenuItem(value: LowFreqHighPass.hz10, child: Text('10 Hz')),
                  ],
                  onChanged: (v) => setState(() => _cfg = _cfg.copyWith(lowFreqHpf: v ?? _cfg.lowFreqHpf)),
                ),
              ),
            ],
          ),
              ...
              Expanded(
                child: DropdownButton<int>(
                  ...
                  items: List.generate(10, (i) => i)
                      ...
                      .toList(),
                  onChanged: (v) => setState(() => _cfg = _cfg.copyWith(decimation: v ?? _cfg.decimation)),
                ),
              ),
            ],
          ),
              ...
              Expanded(
                child: DropdownButton<AudioSource>(
                  ...
                  items: const [
                    ...
                    DropdownMenuItem(value: AudioSource.unprocessed, child: Text('UNPROCESSED (recommandé)')),
              ...
              Expanded(
                child: DropdownButton<DcFilterType>(
                  ...
                  items: const [
                    ...
                    DropdownMenuItem(value: DcFilterType.iir, child: Text('IIR 1er ordre (~3Hz)')),
              ...
              Expanded(
                child: DropdownButton<NotchFilter>(
                  ...
                  items: const [
                    ...
                    DropdownMenuItem(value: NotchFilter.hz50, child: Text('50 Hz (Q=30)')),
                    DropdownMenuItem(value: NotchFilter.hz60, child: Text('60 Hz (Q=30)')),
                  ],
          ...
          SwitchListTile(
            ...
            subtitle: const Text('Désactive AGC, NS et AEC pour analyse précise'),
