...
class _ResearchScreenState extends State<ResearchScreen> {
  ...
  void _updateLockDisplay(String trackerState, double f0Tracked) {
    ...
    final f0Changed = (f0Tracked - _lastF0Tracked).abs() > 1.0; // Plus de 1Hz de changement
        ...
        appBar: AppBar(
          ...
          actions: [
            Builder(builder: (context) {
              ...
              final hzPerBin = st.effectiveSampleRate > 0 && cfg.fftSize > 0 ? (st.effectiveSampleRate / cfg.fftSize).toStringAsFixed(1) : '—';
              return Center(
                child: Padding(
                  padding: const EdgeInsets.symmetric(horizontal: 12),
                  child: Text('FS eff: ${st.effectiveSampleRate} Hz$decimText · Hz/bin@DC: $hzPerBin · $unit'),
                ),
              );
            }),
            Builder(builder: (context) {
                ...
                children: [
                  ...
                  Positioned(
                    ...
                    child: Container(
                      ...
                      child: _AudioStatusWidget(audioStatus: st.audioStatus),
                    ),
                  ),
                  ...
                  Positioned(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        if (st.f0Tracked > 0)
                          Text(
                            'f0: ${st.f0Tracked.toStringAsFixed(1)} Hz',
                            style: const TextStyle(color: Colors.white70, fontSize: 12),
                          ),
                        const SizedBox(height: 6),
                        // Debug overlay for dominant tracker
                        if (st.debugPeaks.isNotEmpty) ...[
                          const Text('Top Peaks:', style: TextStyle(color: Colors.white70, fontSize: 10)),
                          for (int i = 0; i < st.debugPeaks.length.clamp(0, 3); i++)
                            Text(
                              '${(i + 1)}: ${st.debugPeaks[i].freq.toStringAsFixed(1)}Hz, ${st.debugPeaks[i].snr.toStringAsFixed(1)}dB',
                        ...
                        if (st.predictedF0 > 0)
                          Text(
                            'Pred: ${st.predictedF0.toStringAsFixed(1)}Hz, W±${st.currentWindow.toStringAsFixed(0)}¢',
                            style: const TextStyle(color: Colors.white60, fontSize: 9),
                          ),
                      ],
                    ),
                      ...
                      child: Container(
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            Text(
                              '${st.f0Tracked.toStringAsFixed(1)}',
                              style: TextStyle(
                                color: Colors.green,
                                fontSize: 28,
                                fontWeight: FontWeight.bold,
                  ...
                  Positioned.fill(
                    child: Padding(
                      padding: const EdgeInsets.only(top: 110), // Make room for status + chips
                      child: Center(
                        child: SizedBox(
        ...
        Expanded(
          child: Row(
            children: [
              ...
              Text(
                audioStatus.unprocessedAvailable ? 'UNPROCESSED' : 'VOICE_RECOGNITION',
                style: textStyle?.copyWith(
                  color: audioStatus.unprocessedAvailable ? Colors.green : Colors.orange,
                  fontWeight: FontWeight.bold,
                ),
              ),
...
Widget _pitchChip(String label, double f0, double conf) {
  ...
  final text = has ? '${f0.toStringAsFixed(1)} Hz (${(conf*100).toStringAsFixed(0)}%)' : '—';
  return Chip(
